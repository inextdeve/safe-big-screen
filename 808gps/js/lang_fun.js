...
function loadChangeVelocity(lang_, oldType, newType) {
    for (var a in lang_) {
        if (a != 'velocityType_km' && a != 'velocityType_mile' && a != 'velocityType_nautical_mile'
            && a != 'KmPerHourParam_' && a != 'MilePerHourParam_' && a != 'NauticalPerHourParam_' && a != 'policeName' && a != 'enforcementName' && a != 'personName' && !a.endsWith('_v9')) {
...
function loadChangeWord(lang_, oldType, newType) {
    for (var a in lang_) {
        if (a != 'velocityType_km' && a != 'velocityType_mile' && a != 'velocityType_nautical_mile'
            && a != 'KmPerHourParam_' && a != 'MilePerHourParam_' && a != 'NauticalPerHourParam_') {
            lang_[a] = lang_[a].replace(new RegExp(oldType, 'g'), newType);
        }
    }
}
